...
class InstagramBot:
    ...
    # Function willl log us in to Instagram
    def login(self):
        bot = self.bot
        ...
        # Wait for 1 second the press ENTER
        time.sleep(1)
        bot.find_element_by_name('password').send_keys(Keys.RETURN)

        # Wait for 3 seconds while the post-login page loads
        time.sleep(3)
    ...
    def followTheirFollowers(self, number_to_follow):
        ...
        for follower in self.followers:
            ...
            if (len(bot.find_elements_by_xpath("//*[contains(text(), 'This Account is Private')]"))):
                continue
...
insta = InstagramBot('ngochung.ly', 'gslnhung')
insta.login()
